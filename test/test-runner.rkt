...
(define (test-runner run)
  ...
  (check-equal? (run '(vector-ref (make-vector 0 #f) 0)) "ERROR: vector-ref")
  (check-equal? (run '(vector-ref (make-vector 3 5) -1)) "ERROR: vector-ref")
  ...
  (check-equal? (run '(vector-ref (make-vector 3 5) 3)) "ERROR: vector-ref")
  ...
  (check-equal? (run '(string-ref "" 0)) "ERROR: string-ref")
  (check-equal? (run '(string-ref (make-string 0 #\a) 0)) "ERROR: string-ref")
  (check-equal? (run '(string-ref "fred" 0)) #\f)
  (check-equal? (run '(string-ref "fred" 1)) #\r)
  (check-equal? (run '(string-ref "fred" 2)) #\e)
  (check-equal? (run '(string-ref "fred" 4)) "ERROR: string-ref")
  ...
  (check-equal? (run '(error "b"))
                "Error type: b")
  (check-equal? (run '(error? "a")) #f)
  (check-equal? (run '(error? (error "a"))) #t)
  (check-equal? (run '(error? (error 3))) "ERROR: error: need string")
  (check-equal? (run '(error? (raise (error "a")))) "ERROR: a")
  (check-equal? (run '(try-catch (raise (error "a")) err (error? err))) #t)
  (check-equal? (run
                 '(define (f x y)
                    (+ x y))
                 '(let ((z 2)) (f 1 z)))
  ...
  (check-equal? (run 
   ...
   (check-equal? (run 
                 ...
                 "ERROR: lambda: arity mismatch")
   (check-equal? (run '(7)) "ERROR: apply: not a procedure")
   (check-equal? (run '(2 3 4)) "ERROR: apply: not a procedure")
  )
